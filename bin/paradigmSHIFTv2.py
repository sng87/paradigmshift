...
class jtData(Target):
    ...
    def run(self):
        ...
        ## write data
        mData.rwCRSData("full_%s" % (re.split("/", cnvFile)[-1]), cnvFile, useRows = self.dataSamples)
        mData.rwCRSData("full_%s" % (re.split("/", expFile)[-1]), expFile, useRows = self.dataSamples)
        mData.rwCRSData("up_%s" % (re.split("/", cnvFile)[-1]), cnvFile, useRows = self.dataSamples, useCols = self.proteinFeatures)
        mData.rwCRSData("up_%s" % (re.split("/", expFile)[-1]), expFile, useRows = self.dataSamples, useCols = self.proteinFeatures)
        mData.rwCRSData("down_%s" % (re.split("/", cnvFile)[-1]), cnvFile, useRows = self.dataSamples, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        mData.rwCRSData("down_%s" % (re.split("/", expFile)[-1]), expFile, useRows = self.dataSamples, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        ...
        mData.rwCRSData("full_%s" % (re.split("/", cnvFile)[-1]), cnvFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap)
        mData.rwCRSData("full_%s" % (re.split("/", expFile)[-1]), expFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap)
        mData.rwCRSData("up_%s" % (re.split("/", cnvFile)[-1]), cnvFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("up_%s" % (re.split("/", expFile)[-1]), expFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("down_%s" % (re.split("/", cnvFile)[-1]), cnvFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        mData.rwCRSData("down_%s" % (re.split("/", expFile)[-1]), expFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
...
class jtNData(Target):
    ...
    def run(self):
        ...
        ## write data
        mData.rwCRSData("up_N%s_%s" % (self.null, re.split("/", cnvFile)[-1]), cnvFile, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("up_N%s_%s" % (self.null, re.split("/", expFile)[-1]), expFile, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("down_N%s_%s" % (self.null, re.split("/", cnvFile)[-1]), cnvFile, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        mData.rwCRSData("down_N%s_%s" % (self.null, re.split("/", expFile)[-1]), expFile, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        ...
        ## write simulated normals
        mData.rwCRSData("up_N%s_%s" % (self.null, re.split("/", cnvFile)[-1]), cnvFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("up_N%s_%s" % (self.null, re.split("/", expFile)[-1]), expFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = self.proteinFeatures)
        mData.rwCRSData("down_N%s_%s" % (self.null, re.split("/", cnvFile)[-1]), cnvFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
        mData.rwCRSData("down_N%s_%s" % (self.null, re.split("/", expFile)[-1]), expFile, rowMap = rowMap, useRows = rcMap.keys(), rcMap = rcMap, useCols = list(set(self.proteinFeatures)-set([self.mutatedGene])))
...
def main():
    ...
    dataFeatures = list(set(mData.retColumns(cnvFile)) & set(mData.retColumns(expFile)))
    includeFeatures = None
    if len(featureFile) != 0:
        includeFeatures = mData.rList(featureFile)
    
    dataSamples = list(set(mData.retRows(cnvFile)) & set(mData.retRows(expFile)))
    if len(sampleFile) != 0:
        dataSamples = list(set(dataSamples) & set(mData.rList(sampleFile)))
